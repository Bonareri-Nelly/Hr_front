...
export default function AnnouncementsTrainingPage({ employeeOnly = false }: { employeeOnly?: boolean }) {
  ...
  const visibleAnnouncements = employeeOnly ? announcements.data?.filter((item) => String(item.audience ?? "").toLowerCase() !== "management") : announcements.data;
  const visibleTrainings = employeeOnly ? trainings.data?.filter((item) => String(item.status ?? "").toLowerCase() !== "archived") : trainings.data;
  return <div className="dashboard-page" style={{ display: "grid", gap: 18 }}>
    <div><h1 className="page-title">Announcements & Training</h1><p className="page-subtitle">{employeeOnly ? "Your HR announcements and available training sessions." : "Live messages and training sessions from HR Operations."}</p></div>
