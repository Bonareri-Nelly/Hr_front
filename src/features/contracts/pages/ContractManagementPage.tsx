...
export default function ContractManagementPage() {
  ...
  const terminate = useMutation({ mutationFn: (id: number) => actions.terminateContract(id, { termination_type: "Resignation", termination_date: new Date().toISOString().slice(0, 10), reason: "Recorded from contract management" }), onSuccess: () => { setNotice("Contract terminated."); client.invalidateQueries({ queryKey: ["contracts"] }); } });
  const rows = useMemo(() => (contracts.data ?? []).filter((item) => `${text(item, "id")} ${text(item, "employee_name")} ${text(item, "status")}`.toLowerCase().includes(query.toLowerCase())), [contracts.data, query]);
  ...
  return <div className="dashboard-page" style={{ display: "grid", gap: 18 }}>
    ...
    <section className="panel"><div className="panel-header" style={{ display: "flex", gap: 12, justifyContent: "space-between", flexWrap: "wrap" }}><h2 className="panel-title">Employment contracts</h2><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search employee or contract" /></div><div className="panel-body table-wrap"><table className="table"><thead><tr><th>Contract</th><th>Employee</th><th>Type</th><th>Period</th><th>Salary</th><th>Status</th><th>Actions</th></tr></thead><tbody>{contracts.isLoading ? <tr><td colSpan={7}>Loading contracts…</td></tr> : rows.length ? rows.map((item) => <tr key={item.id}><td>CTR-{String(item.id).padStart(5, "0")}</td><td>{text(item, "employee_name")}</td><td>{text(item, "contract_type")}</td><td>{text(item, "start_date")} – {text(item, "end_date")}</td><td>{money(item.gross_salary)}</td><td>{text(item, "status")}</td><td style={{ display: "flex", gap: 6 }}><button className="button button-secondary" disabled={renew.isPending} onClick={() => { const endDate = window.prompt("New end date (YYYY-MM-DD; leave blank for open-ended):", text(item, "end_date").replace("—", "")); const salary = window.prompt("New monthly salary:", String(item.gross_salary ?? "")); if (salary !== null && endDate !== null) renew.mutate({ id: Number(item.id), endDate, salary }); }}>Renew</button><button className="button button-secondary" disabled={terminate.isPending} onClick={() => { if (window.confirm("Terminate this contract?")) terminate.mutate(Number(item.id)); }}>Terminate</button></td></tr>) : <tr><td colSpan={7}>No contracts found.</td></tr>}</tbody></table></div></section>
  </div>;
}
